...
// Version: 4.4.0 - Cache Version 28 (Designer Weekly Hours Analytics)
// ============================================

const CACHE_NAME = 'ebtracker-v28';
const STATIC_CACHE = 'ebtracker-static-v28';
const DYNAMIC_CACHE = 'ebtracker-dynamic-v28';
...
self.addEventListener('install', (event) => {
  console.log('🔧 Service Worker v28: Installing...');
  
  event.waitUntil(
    caches.open(STATIC_CACHE)
      .then((cache) => {
        console.log('📦 Service Worker v28: Caching static assets');
        return cache.addAll(STATIC_ASSETS);
      })
      .then(() => {
        console.log('✅ Service Worker v28: Static assets cached');
        return self.skipWaiting();
      })
      .catch((error) => {
        console.error('❌ Service Worker v28: Cache failed', error);
      })
  );
});
...
self.addEventListener('activate', (event) => {
  console.log('🚀 Service Worker v28: Activating...');
  ...
  event.waitUntil(
    caches.keys()
        ...
        return Promise.all(
          cacheNames.map((cacheName) => {
            ...
            if (!validCaches.includes(cacheName)) {
              console.log('🗑️ Service Worker v28: Deleting old cache:', cacheName);
      ...
      .then(() => {
        console.log('✅ Service Worker v28: Activated - Old caches cleared');
      ...
      .then((clients) => {
        console.log('📢 Service Worker v28: Notifying clients to refresh');
...
self.addEventListener('message', (event) => {
  ...
  switch (event.data.type) {
    ...
    case 'GET_VERSION':
      event.ports[0]?.postMessage({ version: 'v28', cache: CACHE_NAME });
      break;
...
console.log('✅ Service Worker v28: Loaded successfully - Designer Weekly Hours Analytics enabled');
